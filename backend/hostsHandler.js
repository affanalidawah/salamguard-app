...
const checkBlocklistIntegrity = async (callback) => {
  try {
    ...
    fs.readFile(hostsPath, "utf-8", (err, data) => {
      ...
      // Collect existing hosts entries
      const normalizedHostsEntries = new Set(
        dataLines
          .filter(
            (line) => line.startsWith("127.0.0.1") || line.startsWith("0.0.0.0")
          )
          .map((line) => line.split(/\s+/)[1]?.replace(/^www\./, ""))
          .filter(Boolean) // Ensure no undefined entries
      );

      // Check missing domains
      const missingDomains = blocklist.filter(
        (domain) => !normalizedHostsEntries.has(domain)
      );

      console.log("Domains that need to be added", missingDomains.length);

      // if (
      //   missingDomains.length < blocklist.length
      // ) {
      ...
      const urlsFound = specificUrls.every((url) =>
        dataLines.some(
          (line) => line === `127.0.0.1 ${url}` || line === `0.0.0.0 ${url}`
        )
      );

      console.log("startFound", startFound);
      console.log("endFound", endFound);
      console.log("allUrlsFound", allUrlsFound);
